...
import { getStudentFeedOfferings, getCoursePosts, addReply, deleteReply, downloadPostAttachment } from '../../services/apiService';
...
function PostCard({ post, currentUserId, onReply, onDeleteReply, onDownload }) {
    ...
    <div className="bg-white border border-gray-200 rounded-2xl shadow-sm overflow-hidden">
      <div className="px-5 pt-4 pb-3 flex items-start gap-3">
        ...
        <div className="flex-1">
          ...
          <p className="text-sm text-gray-800 mt-2 whitespace-pre-wrap leading-relaxed">{post.body}</p>
        </div>
      </div>
...
export default function StudentCourseFeed() {
  const { user } = useAuth();
  ...
  useEffect(() => {
    getStudentFeedOfferings().then(r => setOfferings(r.data || [])).catch(() => {});
  }, []);
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-gray-500 text-sm mt-1">Posts and resources shared by your instructors.</p>
      </div>
      ...
      {offerings.length > 0 && (
        <div className="flex gap-2 flex-wrap">
          {offerings.map(o => (
            ...
            >
              {o.course?.code}
              <span className="ml-1.5 text-xs opacity-70">{o.semester} {o.year}</span>
            </button>
          ))}
        </div>
      )}
      ...
      {!selectedId && (
        <div className="bg-white border border-gray-200 rounded-2xl p-8 text-center shadow-sm">
          <p className="text-gray-400 text-sm">
            {offerings.length === 0 ? 'You are not enrolled in any courses.' : 'Select a course above to view its feed.'}
          </p>
        </div>
      )}
      ...
      {selectedId && (
        <div className="space-y-1 mb-2">
          <p className="text-xs font-semibold text-gray-500 uppercase tracking-wider">
            {selected?.course?.code} — {selected?.course?.name} · {selected?.semester} {selected?.year}
          </p>
        </div>
      )}
      ...
      {selectedId && (
        loading ? (
          ...
          <div className="bg-white border border-gray-200 rounded-2xl p-8 text-center shadow-sm">
            <p className="text-gray-400 text-sm">No posts yet for this course.</p>
          </div>
        ) : (
